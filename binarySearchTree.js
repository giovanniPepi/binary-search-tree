...
class Tree {
  constructor(array) {
    // first we sort our array using mergeSort
  ...
  buildTree(array, start, end) {
    ...
    // then we find the middle of the array
    let mid = parseInt((start + end) / 2);
    // and create a root node with the mid
    let root = new Node(array[mid]);

    //and finally recursively call buildtree to build the left/right part,
    //if any
  ...
  find(value, root = this.root) {
    ...
    // recursively looks for the value
    if (root.data > value) {
